...
# Optional Excel-open support (Windows desktop Excel)
try:
...
# PAGE CONFIG
# =============================
st.set_page_config(page_title="Relationship Team Leave Tracker (Gurgaon)", layout="wide")
...
LEAVE_TYPES = ["PL", "PL 1/2", "SL", "SL 1/2", "PH", "PAT", "ED", "BL", "LWP", "SH", "Present"]
LEAVE_ONLY_TYPES = ["PL", "PL 1/2", "SL", "SL 1/2", "PH", "PAT", "ED", "BL", "LWP", "SH"]
...
st.markdown(
    '<div class="main-title">📊 Relationship Team Leave Tracker (Gurgaon)</div>',
    unsafe_allow_html=True
)
st.markdown(
    '<div class="subtle">Advanced version with correction mode, undo last update, employee leave history, employee management, filters, and open-Excel support.</div>',
    unsafe_allow_html=True
)
...
def safe_save_workbook(wb, file_path: Path):
    ...
    tmp_fd, tmp_name = tempfile.mkstemp(suffix=file_path.suffix, dir=str(file_path.parent))
    os.close(tmp_fd)
    try:
        wb.save(tmp_name)
...
def get_open_xlwings_book(file_path: Path):
    """
    Return an already-open Excel workbook handle if the workbook is open in desktop Excel.
    """
...
def get_emp_summary(long_df: pd.DataFrame):
    ...
    summary = long_df.groupby(["Emp ID", "Name", "Team", "Designation"], dropna=False).agg(
...
def apply_tracker_changes(tracker_path: Path, changes):
    """
    Apply tracker changes. Each change dict must contain:
    emp_id, date, value
    """
    ...
    if open_book is not None:
        ...
        for ch in changes:
            emp_id = str(ch["emp_id"])
            target_date = ch["date"]
            new_value = "" if ch["value"] == "Present" else ch["value"]

            if is_weekend_date(target_date):
                continue

            if emp_id not in emp_row_cache:
                emp_row = None
                for r in range(2, len(values) + 1):
                    row_vals = values[r - 1]
                    cell_val = row_vals[emp_col - 1] if row_vals else None
                    if is_emp_id(cell_val) and normalize_emp_id(cell_val) == emp_id:
                        emp_row = r
                        break
                if emp_row is None:
                    continue
                emp_row_cache[emp_id] = emp_row

            col = header_map.get(target_date)
            if col is None:
                continue

            ws.range((emp_row_cache[emp_id], col)).value = new_value
    ...
    for ch in changes:
        ...
        ws.cell(row=emp_row_cache[emp_id], column=col).value = new_value

    safe_save_workbook(wb, tracker_path)


def update_tracker_workbook(tracker_path: Path, emp_id: str, start_date: date, end_date: date, leave_type: str):
    """
    Update tracker and return detailed change log for undo.
    """
    missed_dates = []
    skipped_weekends = []
    updated_dates = []
...
def add_employee_to_tracker(tracker_path: Path, emp_id: str, name: str, joining_date_value: str, team: str, designation: str):
    open_book = get_open_xlwings_book(tracker_path)
    ...
    header_lookup = {str(ws.cell(row=1, column=c).value).strip(): c for c in range(1, ws.max_column + 1)}
...
def add_employee_to_preferred(pref_path: Path, emp_id: str, name: str, joining_date_value: str, team: str):
    if not pref_path.exists():
        return
...
base_dir = st.sidebar.text_input("Data folder path", str(DEFAULT_BASE_DIR.resolve()))
...
if team_filter_sidebar.strip():
    tracker_df = tracker_df[tracker_df["Team"].astype(str).str.contains(team_filter_sidebar, case=False, na=False)].copy()
    long_df = long_df[long_df["Team"].astype(str).str.contains(team_filter_sidebar, case=False, na=False)].copy()
    summary_df = summary_df[summary_df["Team"].astype(str).str.contains(team_filter_sidebar, case=False, na=False)].copy()

if employee_search_sidebar.strip():
    tracker_df = tracker_df[tracker_df["Name"].astype(str).str.contains(employee_search_sidebar, case=False, na=False)].copy()
    long_df = long_df[long_df["Name"].astype(str).str.contains(employee_search_sidebar, case=False, na=False)].copy()
    summary_df = summary_df[summary_df["Name"].astype(str).str.contains(employee_search_sidebar, case=False, na=False)].copy()
...
if menu == "Live Dashboard":
    ...
    if not on_leave_df.empty:
        st.markdown("### 📊 Leave Count by Team")
        team_counts = on_leave_df.groupby("Team").size().rename("Employees on Leave").reset_index()
...
# =============================
    ...
    st.caption(
        "You can correct mistakes from this screen itself. "
        "If leave was applied to the wrong employee, wrong date, or wrong leave type, update the same entry here. "
        "Select Leave Type = Present to remove incorrect leave."
    )

    with st.form("leave_form", clear_on_submit=False):
        emp_name = st.selectbox("Select Employee", tracker_df["Name"].dropna().sort_values().tolist())
    ...
    st.markdown(
        "- If leave was applied to the wrong employee, select that employee, choose the same date, and set Leave Type = Present.\n"
        "- If leave was applied on the wrong date, select that date and set Leave Type = Present.\n"
        "- If the wrong leave type was selected, update it with the correct leave type.\n"
        "- After saving, the dashboard will auto-refresh and the 'On Leave Today' count will update automatically."
    )
    ...
    if submitted:
        if d_start > d_end:
            ...
            try:
                ...
                if updated_dates:
                    st.write("Updated dates:", ", ".join(updated_dates[:15]) + (" ..." if len(updated_dates) > 15 else ""))
                if skipped_weekends:
                    st.warning(
                        "Weekend dates were automatically skipped (leave cannot be marked on weekends): "
                        + ", ".join(skipped_weekends[:10])
                        + (" ..." if len(skipped_weekends) > 10 else "")
                    )
                if missed_dates:
                    st.warning("These dates were not found in the tracker columns: " + ", ".join(missed_dates))
...
# UNDO LAST UPDATE
# =============================
elif menu == "Undo Last Update":
    st.markdown("### ↩️ Undo Last Update")

    if not st.session_state.undo_stack:
        st.info("No updates are available to undo in the current app session.")
    else:
        last_entry = st.session_state.undo_stack[-1]
        st.write("Last updated employee:", last_entry["employee_name"])

        preview = pd.DataFrame([
            {
                "Date": ch["date"],
                "Old Value": ch["old_value"] or "Present/Blank",
                "New Value": ch["new_value"] or "Present/Blank"
            }
            for ch in last_entry["change_log"]
        ])
                ...
                revert_changes = [
                    {
                        "emp_id": ch["emp_id"],
                        "date": ch["date"],
                        "value": ch["old_value"] if ch["old_value"] else "Present"
                    }
                    for ch in last_entry["change_log"]
                ]

                apply_tracker_changes(tracker_path, revert_changes)

                if pref_path.exists():
                    sync_preferred_from_change_log(pref_path, last_entry["change_log"], use_new_value=False)

                st.session_state.undo_stack.pop()
                clear_caches()
                st.success("The last update has been undone successfully.")
                st.rerun()
...
# =============================
    ...
    if tracker_df.empty:
        ...
        hist_df = long_df[
            ...
            (long_df["Status"].astype(str).str.strip() != "")
        ].copy()

        if hist_df.empty:
            st.info("No leave history is available for the selected employee.")
        else:
            hist_df = hist_df[["Date", "Status", "Team", "Designation"]].sort_values("Date", ascending=False)
            hist_df.columns = ["Date", "Leave Type", "Team", "Designation"]
            st.dataframe(hist_df, use_container_width=True, hide_index=True)

            month_breakdown = hist_df.copy()
            month_breakdown["Month"] = pd.to_datetime(month_breakdown["Date"]).dt.to_period("M").astype(str)
            st.markdown("#### Monthly Leave Count")
            st.bar_chart(month_breakdown.groupby("Month").size())
        ...
        if add_submitted:
            if not str(new_emp_id).strip().isdigit():
                ...
                try:
                    ...
                    clear_caches()
                    st.success(f"Employee {new_name} ({new_emp_id}) has been added successfully.")
                    st.rerun()
        ...
        if tracker_df.empty:
            ...
            if st.button("🗑️ Remove Employee", disabled=not confirm_remove):
                try:
                    ...
                    clear_caches()
                    st.success(f"Employee {remove_name} ({remove_emp_id}) has been removed successfully.")
                    st.rerun()
...
# =============================
    ...
    out = tracker_df[["Emp ID", "Name", "Joining Date", "Team", "Designation"]].copy()

    if not pref_df.empty and "Reporting Manager" in pref_df.columns:
...
# =============================
    ...
    display_summary = summary_df[[
        ...
        "PL", "PL_Half", "SL", "SL_Half", "PH", "PAT", "ED", "BL", "LWP", "SH",
        "Total_Leave_Days"
    ]].copy()

    display_summary.columns = [
        "Employee Name", "Team", "Designation",
        "PL", "PL 1/2", "SL", "SL 1/2", "PH", "PAT", "ED", "BL", "LWP", "SH",
        "Total Leave Days"
    ]
